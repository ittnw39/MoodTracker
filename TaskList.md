...
- [x] UI 디테일 조정:
    ...
    - [x] `MainActivity` 달력 UI 이전 스타일로 복원 (요일 표시, 날짜 칸 높이 및 패딩, 전체적인 그리드 레이아웃, `setTodayHighlightOrSelection` 함수 로직 복원)
...
세부 작업:
- [x] 통계 화면 UI 설계 (그래프 표시 영역, 기간 선택 옵션 등) - 기본 PieChart 및 Toolbar 추가 완료.
...
- [x] 통계 기능 (`StatsActivity`)에 사용자 정의 감정 반영 및 삭제된 감정 제외 처리
...
세부 작업:
- [x] 사용자 정의 감정 추가/수정/삭제 UI 및 기본 로직 구현 (`ManageMoodsActivity`)
- [x] 색상 선택기 라이브러리 변경 (`com.jaredrummler:colorpicker`) 및 적용
- [x] 감정 수정 시 `mood_data.txt`의 기존 기록된 색상 동기화
- [x] 감정 삭제 시 `mood_data.txt`의 기존 기록 삭제 동기화
- [x] 버그 수정: `ManageMoodsActivity`에서 색상 코드 입력 없이 색상 선택기 버튼 클릭 시 발생하는 `StringIndexOutOfBoundsException` 해결
- [x] 기능 개선: `ManageMoodsActivity`에서 감정 수정/삭제 후 `MainActivity` 달력에 즉시 반영 (onResume 로직 추가)
- [x] 최적화: `MainActivity`에서 `mood_data.txt` 파일 중복 로드 방지를 위한 `moodsMap` 멤버 변수 사용
- [x] 감정 데이터 저장 구조 변경 검토 및 설계.
- [x] 감정 관리 UI/UX 설계 (추가/수정/삭제 화면 또는 다이얼로그).
- [x] 감정 추가/수정/삭제 CRUD 로직 구현.
- [x] 기분 기록 다이얼로그, 달력, 통계 기능에 사용자 정의 감정 연동.
...
세부 작업:
- [x] **기간 직접 선택 기능:** 시작 날짜와 종료 날짜를 달력 등으로 직접 선택하여 기간 설정 UI 및 로직 구현. (`StatsActivity`에 날짜 선택 버튼, 선택된 기간 표시 TextView 추가, `DatePickerDialog` 연동, 선택된 기간에 따른 차트 데이터 필터링 로직 구현 완료, 차트 데이터셋 라벨 제거)
...
- [ ] **조절 가능한 바(Bar) UI 설계 및 구현:**
    - 기간을 시각적으로 조절할 수 있는 바(Seekbar 또는 커스텀 UI) 디자인.
    - 바(Bar)에 일/주/월 단위 눈금 표시 로직 구현.
    - 바(Bar) 조작에 따른 기간 변경 및 통계 업데이트 연동.
- [ ] 기존 월별 통계 기능과 새로운 기간 설정 기능 통합 및 UI/UX 개선.
적용 내용: 고급 UI 컨트롤(달력, 커스텀 바), 날짜/시간 라이브러리 활용, 복잡한 기간 필터링 로직, 사용자 경험 최적화.
